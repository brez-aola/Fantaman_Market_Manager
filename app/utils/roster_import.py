...
def apply_roster(db_path, team_players, audit_info: dict | None = None):
    ...
    from app.models import Player, ImportAudit
    ...
    try:
        ...
        for team_name, players in team_players.items():
            ...
            for p in players:
                ...
                existing = s.execute(text('SELECT rowid FROM giocatori WHERE lower("Nome") = lower(:nome)'), {"nome": nome}).fetchone()
                if existing:
                    rowid = existing[0]
                    ...
                    try:
                        s.execute(text(f"UPDATE giocatori SET {set_clause} WHERE rowid=:rowid"), params)
                        updated += 1
                    except Exception:
                        pass
                else:
                    ...
                    if not to_insert:
                        ...
                        try:
                            s.execute(text(f"INSERT INTO giocatori ({cols}) VALUES ({qmarks})"), to_insert)
                            inserted += 1
                        except Exception:
                            pass
        ...
        for team_name in team_players.keys():
            r = s.execute(text("SELECT cassa_iniziale FROM fantateam WHERE squadra=:team"), {"team": team_name}).fetchone()
            starting = float(r[0]) if r and r[0] is not None else 300.0
            spent_row = s.execute(text('SELECT COALESCE(SUM(CAST(REPLACE(REPLACE(REPLACE(COALESCE("Costo", "0"), ",", ""), "%", ""), " ", "") AS REAL)),0) as spent FROM giocatori WHERE squadra=:team AND NOT (opzione = "SI" AND anni_contratto IS NULL)'), {"team": team_name}).fetchone()
            spent = float(spent_row[0]) if spent_row and spent_row[0] is not None else 0.0
            new_attuale = starting - spent
            s.execute(text("INSERT OR REPLACE INTO fantateam(squadra, carryover, cassa_iniziale, cassa_attuale) VALUES (:t,0,:start,:att)"), {"t": team_name, "start": starting, "att": new_attuale})
