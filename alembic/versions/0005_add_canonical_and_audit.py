...
def table_exists(tablename):
    conn = op.get_bind()
    res = conn.execute(sa.text("SELECT name FROM sqlite_master WHERE type='table' AND name=:t"), {"t": tablename}).fetchone()
    return bool(res)
...
def upgrade():
    ...
    if not table_exists('canonical_mappings'):
        op.create_table(
            ...
            sa.Column('variant', sa.String(256), nullable=False, unique=True, index=True),
